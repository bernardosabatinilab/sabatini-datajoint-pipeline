...
def make_bandit_df(df, fracTimeout):
    ...
    df=df[['Mouse', 'Date', 'Session', 'Condition', 'sSelection', 'tSelection', 'I_anySelect_L',
           'I_anySelect_R', 'I_giveReward', 'T_Reward','T_ENL', 'n_ENL','n_Cue', 'DAB_I_flipLR_event',
           'DAB_I_flipLR', 'DAB_I_HighProbSel']] # include only columns of interest
    ...
    counts = np.unique(df['iBlock'], return_counts=True)[1] # number of trials per block
    ...
    df['direction'] = df['I_anySelect_R']                   # make new column called direction where right = 1, left = 0
    df.loc[df.sSelection == 3,'direction'] = np.nan         # and no selection is marked by a NaN.
    ...
    indNotNaN = np.isnan(df.direction.values) == False                # find elements in direction columns that are NOT NaN
    diffDir = abs(np.diff(df.direction[indNotNaN])) # take the difference of consecutive elements, excluding timeouts
    df['Switch'] = 0
    switch = np.insert(diffDir,0,0)
    df.loc[indNotNaN, 'Switch'] = switch # so switches include direction A->timeout->direction B
    
    #df['Switch'] = np.abs(np.diff(df.direction, prepend=df.direction.iloc[0])) # previously                
    df=df.rename(columns={'I_giveReward':'Reward','DAB_I_flipLR':'State', 'DAB_I_HighProbSel': 'selHigh' })
    df.drop(['I_anySelect_L', 'I_anySelect_R','DAB_I_flipLR_event'], axis = 1, inplace=True) # remove redundant columns
...
def get_previous_event(data, eventName, binarize, nBack ):
    ...
    if binarize:
        previous_event = list((data[eventName[0]].values>0).astype('int')) # list binary reward outcome
    else:
...
def get_next_event(data, eventName, binarize, nFor):
    """this currently works for any column that has integer values"""
    if binarize:
        next_event = list((data[eventName[0]].values>0).astype('int')) # list binary reward outcome
    else:
...
def label_history_ab(df, history_length=3):
    ...
    for row in np.arange( (history_length-1), df.shape[0] ): # going row by row to label history     
        
        h='A' if df.loc[row-(history_length-1)].Reward else 'a' # everything is referenced to the row N before
                
        for i in np.arange(history_length-2,-1,-1):
            if df.loc[row-(history_length-1)].direction ==df.loc[row-i].direction:
...
def get_reward_seq(df):
    ...
    df['reward_seq'] = df.Reward.cumsum() - df.Reward.cumsum().where(df.Reward==0).ffill().fillna(0).astype(int)
    df['loss_seq'] = (1-df.Reward).cumsum() - (df.Reward==0).cumsum().where(df.Reward>0).ffill().fillna(0).astype(int) 
